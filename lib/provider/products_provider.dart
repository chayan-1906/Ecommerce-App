...
class ProductsProvider with ChangeNotifier {
  ...
  List<Product> findByCategory(String categoryName) {
    List<Product> _categoryList = _productList
  ...
  List<Product> findByBrand(String brandName) {
    List<Product> _brandList = _productList
        .where((element) =>
  ...
  List<Product> get findByPopular {
    List<Product> _popularList =
        _productList.where((element) => element.isPopular).toList();
    return _popularList;
  }
  ...
  List<Product> searchQuery(String searchText) {
    List<Product> _searchList = _productList
        .where((element) =>
  ...
  Future<void> fetchProducts() async {
    await FirebaseFirestore.instance
      ...
      _productList = [];
      productSnapshot.docs.forEach((elementProduct) {
        _productList.insert(
          0,
          Product(
            id: (elementProduct.data() as dynamic)['productId'] ?? '',
            title: (elementProduct.data() as dynamic)['productTitle'] ?? '',
            description:
                (elementProduct.data() as dynamic)['productDescription'] ?? '',
            price: (elementProduct.data() as dynamic)['price'] ?? '',
            imageUrl: (elementProduct.data() as dynamic)['productImage'] ?? '',
            productCategoryName:
                (elementProduct.data() as dynamic)['productCategory'] ?? '',
            brand: (elementProduct.data() as dynamic)['productBrand'] ?? '',
            quantity:
                (elementProduct.data() as dynamic)['productQuantity'] ?? '',
            isFavorite: false,
            isPopular: true,
          ),
        );
